...
const FollowerModal = () => {
  const isOpen = useSelector((state) => state.modal.followers);
  const [searchQuery, setSearchQuery] = useState("");
  const [searchParams, setSearchParams] = useSearchParams();
  const username = searchParams.get("followers_req_username") || null;

  if (!username) {
    handleHideModal("followers");
    return;
  }
  ...
  const closeModal = () => {
    ...
    setSearchParams((prev) => {
      return prev.delete("followers_req_username");
    });
  };
